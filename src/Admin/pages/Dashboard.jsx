import React from "react";

function Dashboard() {
  return (
        <div className="content-body">
            <div className="container-fluid">
                {/* ===== Stats Row ===== */}
                <div className="row">
                    {/* New Booking */}
                    <div className="col-xl-3 col-sm-6">
                        <div className="card gradient-1 card-bx">
                            <div className="card-body d-flex align-items-center">
                                <div className="me-auto text-white">
                                    <h2 className="text-white">872</h2>
                                    <span className="fs-18">New Booking</span>
                                </div>
                                <svg width="58" height="58" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        fillRule="evenodd"
                                        clipRule="evenodd"
                                        d="M29.0611 39.4402L13.7104 52.5947C12.9941 53.2089 11.9873 53.3497 11.1271 52.9556C10.2697 52.5614 9.7226 51.7041 9.7226 50.7597V14.5028C9.7226 9.16424 14.0517 4.83655 19.3904 4.83655H38.7289C44.0704 4.83655 48.3995 9.16424 48.3995 14.5028V50.7597C48.3995 51.7041 47.8495 52.5614 46.9922 52.9556C46.1348 53.3497 45.1252 53.2089 44.4088 52.5947L29.0611 39.4402Z"
                                        fill="white"
                                    />
                                </svg>
                            </div>
                        </div>
                    </div>

                    {/* Schedule Room */}
                    <div className="col-xl-3 col-sm-6">
                        <div className="card gradient-2 card-bx">
                            <div className="card-body d-flex align-items-center">
                                <div className="me-auto text-white">
                                    <h2 className="text-white">285</h2>
                                    <span className="fs-18">Schedule Room</span>
                                </div>
                                <svg width="58" height="58" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        fillRule="evenodd"
                                        clipRule="evenodd"
                                        d="M36.25 9.66665V7.24998C36.25 5.91598 37.3327 4.83331 38.6667 4.83331C40.0007 4.83331 41.0833 5.91598 41.0833 7.24998V9.66665C46.4242 9.66665 50.75 13.9949 50.75 19.3333V43.5C50.75 48.8384 46.4242 53.1666 41.0833 53.1666H16.9167C11.5782 53.1666 7.25 48.8384 7.25 43.5V19.3333C7.25 13.9949 11.5782 9.66665 16.9167 9.66665V7.24998C16.9167 5.91598 17.9993 4.83331 19.3333 4.83331C20.6673 4.83331 21.75 5.91598 21.75 7.24998V9.66665H36.25Z"
                                        fill="white"
                                    />
                                </svg>
                            </div>
                        </div>
                    </div>

                    {/* Check In */}
                    <div className="col-xl-3 col-sm-6">
                        <div className="card gradient-3 card-bx">
                            <div className="card-body d-flex align-items-center">
                                <div className="me-auto text-white">
                                    <h2 className="text-white">53</h2>
                                    <span className="fs-18">Check In</span>
                                </div>
                                <svg width="58" height="58" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        fillRule="evenodd"
                                        clipRule="evenodd"
                                        d="M9.66671 38.6667V43.5C9.66671 48.8409 13.995 53.1667 19.3334 53.1667H43.5C48.8409 53.1667 53.1667 48.8409 53.1667 43.5C53.1667 35.455 53.1667 22.5475 53.1667 14.5C53.1667 9.16162 48.8409 4.83337 43.5 4.83337H19.3334C13.995 4.83337 9.66671 9.16162 9.66671 14.5V19.3334C9.66671 20.6674 10.7494 21.75 12.0834 21.75C13.4174 21.75 14.5 20.6674 14.5 19.3334V14.5C14.5 11.832 16.6654 9.66671 19.3334 9.66671H43.5C46.1705 9.66671 48.3334 11.832 48.3334 14.5V43.5C48.3334 46.1705 46.1705 48.3334 43.5 48.3334H19.3334C16.6654 48.3334 14.5 46.1705 14.5 43.5V38.6667C14.5 37.3351 13.4174 36.25 12.0834 36.25C10.7494 36.25 9.66671 37.3351 9.66671 38.6667ZM27.9995 26.5834L24.8748 23.461C23.9323 22.5161 23.9323 20.9864 24.8748 20.0415C25.8197 19.099 27.3495 19.099 28.292 20.0415L35.542 27.2915C36.4869 28.2364 36.4869 29.7661 35.542 30.711L28.292 37.961C27.3495 38.9035 25.8197 38.9035 24.8748 37.961C23.9323 37.0161 23.9323 35.4864 24.8748 34.5415L27.9995 31.4167H7.25004C5.91604 31.4167 4.83337 30.334 4.83337 29C4.83337 27.6685 5.91604 26.5834 7.25004 26.5834H27.9995Z"
                                        fill="white"
                                    />
                                </svg>
                            </div>
                        </div>
                    </div>

                    {/* Check Out */}
                    <div className="col-xl-3 col-sm-6">
                        <div className="card gradient-4 card-bx">
                            <div className="card-body d-flex align-items-center">
                                <div className="me-auto text-white">
                                    <h2 className="text-white">78</h2>
                                    <span className="fs-18">Check Out</span>
                                </div>
                                <svg width="57" height="46" viewBox="0 0 57 46" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        fillRule="evenodd"
                                        clipRule="evenodd"
                                        d="M8.55512 20.7503L11.4641 17.8435C12.3415 16.9638 12.3415 15.5397 11.4641 14.6601C10.5844 13.7827 9.16031 13.7827 8.28289 14.6601L1.53353 21.4094C0.653858 22.2891 0.653858 23.7132 1.53353 24.5929L8.28289 31.3422C9.16031 32.2197 10.5844 32.2197 11.4641 31.3422C12.3415 30.4626 12.3415 29.0385 11.4641 28.1588L8.55512 25.2498H27.8718C29.1137 25.2498 30.1216 24.2419 30.1216 23C30.1216 21.7604 29.1137 20.7503 27.8718 20.7503H8.55512Z"
                                        fill="white"
                                    />
                                </svg>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="row">
                    <div className="col-xl-3 col-xxl-4">
                        <div className="row">
                            <div className="col-xl-12 col-sm-6">
                                <div className="card text-center">
                                    <div className="card-body">
                                        <div id="radialChart" className="radialChart"></div>
                                        <h2>785</h2>
                                        <span className="fs-16 text-black">Available Room Today</span>
                                    </div>
                                </div>
                            </div>

                            <div className="col-xl-12 col-sm-6">
                                <div className="card">
                                    <div className="card-header border-0 pb-0">
                                        <h4 className="mb-0">Booked Room Today</h4>
                                    </div>
                                    <div className="card-body">
                                        <div className="progress mb-4" style={{ height: "13px" }}>
                                            <div
                                                className="progress-bar gradient-5 progress-animated"
                                                style={{ width: "55%", height: "13px" }}
                                                role="progressbar"
                                            >
                                                <span className="sr-only">45% Complete</span>
                                            </div>
                                        </div>

                                        <div className="progress mb-4" style={{ height: "13px" }}>
                                            <div
                                                className="progress-bar gradient-6 progress-animated"
                                                style={{ width: "70%", height: "13px" }}
                                                role="progressbar"
                                            >
                                                <span className="sr-only">45% Complete</span>
                                            </div>
                                        </div>

                                        <div className="progress default-progress" style={{ height: "13px" }}>
                                            <div
                                                className="progress-bar gradient-7 progress-animated"
                                                style={{ width: "50%", height: "13px" }}
                                                role="progressbar"
                                            >
                                                <span className="sr-only">45% Complete</span>
                                            </div>
                                        </div>

                                        <div className="d-flex mt-4 progress-bar-legend align-items-center justify-content-between">
                                            <div className="d-flex">
                                                <span className="marker gradient-5"></span>
                                                <div>
                                                    <p className="status">Pending</p>
                                                    <span className="result">234</span>
                                                </div>
                                            </div>
                                            <div className="d-flex">
                                                <span className="marker gradient-6"></span>
                                                <div>
                                                    <p className="status">Done</p>
                                                    <span className="result">65</span>
                                                </div>
                                            </div>
                                            <div className="d-flex">
                                                <span className="marker gradient-7"></span>
                                                <div>
                                                    <p className="status">Finish</p>
                                                    <span className="result">763</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="col-xl-9 col-xxl-8">
                        <div className="card">
                            <div className="card-header border-0 d-sm-flex d-block">
                                <div className="me-auto mb-sm-0 mb-3">
                                    <h4 className="card-title mb-2">Reservation Statistic</h4>
                                    <span>Lorem ipsum dolor sit amet</span>
                                </div>
                                <div className="d-flex justify-content-between">
                                    <div className="d-flex me-5">
                                        <h3 className="mb-0 me-2">549</h3>
                                        <span>Check In</span>
                                    </div>
                                    <div className="d-flex me-3">
                                        <h3 className="mb-0 me-2">327</h3>
                                        <span>Check Out</span>
                                    </div>
                                    <div className="dropdown">
                                        <a href="#" className="btn-link" data-bs-toggle="dropdown" aria-expanded="false">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M12 13C12.5523 13 13 12.5523 13 12C13 11.4477 12.5523 11 12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13Z"
                                                    stroke="#575757" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                                                ></path>
                                                <path
                                                    d="M12 6C12.5523 6 13 5.55228 13 5C13 4.44772 12.5523 4 12 4C11.4477 4 11 4.44772 11 5C11 5.55228 11.4477 6 12 6Z"
                                                    stroke="#575757" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                                                ></path>
                                                <path
                                                    d="M12 20C12.5523 20 13 19.5523 13 19C13 18.4477 12.5523 18 12 18C11.4477 18 11 18.4477 11 19C11 19.5523 11.4477 20 12 20Z"
                                                    stroke="#575757" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                                                ></path>
                                            </svg>
                                        </a>
                                        <div className="dropdown-menu">
                                            <a className="dropdown-item" href="#">Delete</a>
                                            <a className="dropdown-item" href="#">Edit</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="card-body">
                                <div id="reservationChart" className="reservationChart"></div>
                            </div>
                        </div>
                    </div>

                    <div className="col-xl-6">
                        <div className="card">
                            <div className="card-body booking-calender pb-0">
                                <input type="text" className="form-control d-none" id="datetimepicker" />
                            </div>
                            <div className="card-body py-0">
                                <div className="d-flex justify-content-between mb-3">
                                    <h4 className="card-title">Newest Booking</h4>
                                    <a href="#" className="btn-link">More</a>
                                </div>
                                <div className="row">
                                    {[1, 2, 3, 4].map((i) => (
                                        <div className="col-xl-6 col-sm-6" key={i}>
                                            <div className="dz-image-bx rounded">
                                                <div className={`dz-media ${i <= 2 ? "active" : ""} me-3`}>
                                                    <img className="rounded" src={`/admin-images/avatar/${i}.jpg`} alt="" />
                                                </div>
                                                <div className="dz-info">
                                                    <h5>{["Samantha Humble", "Louise Marquee", "Richard Smile", "Bella Yen"][i - 1]}</h5>
                                                    <span className="text-primary">October 3th, 2020</span>
                                                    <div className="d-flex flex-wrap mt-3">
                                                        <span className="me-auto pe-3">Room A-21</span>
                                                        <span>3-5 Person</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="col-xl-6">
                        <div className="row">
                            <div className="col-xl-12">
                                <div className="card">
                                    <div className="card-body">
                                        <div className="row">
                                            <div className="col-xl-6 d-flex align-items-center col-sm-6 mb-sm-0 mb-3">
                                                <div className="me-4">
                                                    <span className="donut">6/8</span>
                                                </div>
                                                <div>
                                                    <h2>70%</h2>
                                                    <span className="fs-18">Check In</span>
                                                </div>
                                            </div>
                                            <div className="col-xl-6 d-flex align-items-center col-sm-6">
                                                <div className="me-4">
                                                    <span className="donut">2/8</span>
                                                </div>
                                                <div>
                                                    <h2>30%</h2>
                                                    <span className="fs-18">Check Out</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div className="col-xl-12">
                                <div className="card">
                                    <div className="card-header border-0">
                                        <h4 className="card-title">Latest Customer Review</h4>
                                        <div className="dropdown">
                                            <a href="#" className="btn-link" data-bs-toggle="dropdown" aria-expanded="false">
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M12 13C12.5523 13 13 12.5523 13 12C13 11.4477 12.5523 11 12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13Z"
                                                        stroke="#575757" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
                                                    <path
                                                        d="M12 6C12.5523 6 13 5.55228 13 5C13 4.44772 12.5523 4 12 4C11.4477 4 11 4.44772 11 5C11 5.55228 11.4477 6 12 6Z"
                                                        stroke="#575757" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
                                                    <path
                                                        d="M12 20C12.5523 20 13 19.5523 13 19C13 18.4477 12.5523 18 12 18C11.4477 18 11 18.4477 11 19C11 19.5523 11.4477 20 12 20Z"
                                                        stroke="#575757" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
                                                </svg>
                                            </a>
                                            <div className="dropdown-menu">
                                                <a className="dropdown-item" href="#">Delete</a>
                                                <a className="dropdown-item" href="#">Edit</a>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="card-body customer-review-bx p-0 dz-scroll loadmore-content" id="customerReviewContent">
                                        {[
                                            {
                                                img: "/admin-images/avatar/1.jpg",
                                                name: "Ali Muzair",
                                                date: "Posted on 26/04/2020, 12:42 AM",
                                                stars: 4,
                                            },
                                            {
                                                img: "/admin-images/avatar/2.jpg",
                                                name: "Keanu Repes",
                                                date: "Posted on 26/04/2020, 12:42 AM",
                                                stars: 2,
                                            },
                                            {
                                                img: "/admin-images/avatar/3.jpg",
                                                name: "Chintya Clara",
                                                date: "Posted on 26/04/2020, 12:42 AM",
                                                stars: 3,
                                            },
                                            {
                                                img: "/admin-images/avatar/1.jpg",
                                                name: "Ali Muzair",
                                                date: "Posted on 26/04/2020, 12:42 AM",
                                                stars: 4,
                                            },
                                        ].map((review, index) => (
                                            <div className="dz-review-bx" key={index}>
                                                <img className="dz-media me-4" src={review.img} alt={review.name} />
                                                <div className="dz-info">
                                                    <div className="dz-name mb-3 d-flex justify-content-between">
                                                        <div>
                                                            <h6 className="title mb-1">{review.name}</h6>
                                                            <span className="date text-black op8">{review.date}</span>
                                                        </div>
                                                        <ul className="star-review">
                                                            {[...Array(5)].map((_, i) => (
                                                                <li key={i}>
                                                                    <i className={`fas fa-star ${i < review.stars ? "orange" : ""}`}></i>
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    </div>
                                                    <div className="dz-footer">
                                                        <p className="text-black op8 mb-sm-0 mb-3 me-4">
                                                            I have been there many times. Rooms, Food and Service are excellent. We did lots of excursions,
                                                            and all the places are reachable from the hotel. We visited Long Waterfall — very helpful and excellent.
                                                        </p>
                                                        <a className="review-icon rounded-circle bg-success text-white me-3" href="#">
                                                            <i className="far fa-check-circle"></i>
                                                        </a>
                                                        <a className="review-icon rounded-circle bg-danger text-white" href="#">
                                                            <i className="far fa-times-circle"></i>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>

                                    <div className="card-footer style-1 border-0 pt-0 text-center">
                                        <a href="#" className="dz-load-more fas fa-chevron-down" id="customerReview"></a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>


            </div>
        </div>
  );
}

export default Dashboard;


// import React from "react";
// import StatsCard from "../components/StatsCard";
// import { Link } from "react-router-dom";

// function Dashboard() {
//   return (
//     <div>
      
//       <div className="d-flex justify-content-between align-items-center mb-4">
//         <h3 className="mb-0">Overview</h3>
//         <div>
//           <Link to="/admin/bookings" className="btn btn-outline-primary me-2">View Bookings</Link>
//           <Link to="/admin/rooms" className="btn btn-primary">Manage Rooms</Link>
//         </div>
//       </div>

//       <div className="row g-3 mb-4">
//         <div className="col-md-3"><StatsCard title="Total Bookings" value="320" icon="🧾" /></div>
//         <div className="col-md-3"><StatsCard title="Available Rooms" value="45" icon="🛏️" /></div>
//         <div className="col-md-3"><StatsCard title="Revenue" value="$54,000" icon="💵" /></div>
//         <div className="col-md-3"><StatsCard title="Feedbacks" value="122" icon="⭐" /></div>
//       </div>

//       <div className="card mb-4 shadow-sm">
//         <div className="card-body">
//           <h5 className="mb-3">Recent Bookings</h5>
//           <div className="table-responsive">
//             <table className="table table-hover mb-0">
//               <thead>
//                 <tr><th>#</th><th>Guest</th><th>Room</th><th>Check-in</th><th>Check-out</th><th>Status</th></tr>
//               </thead>
//               <tbody>
//                 <tr><td>001</td><td>John Doe</td><td>Deluxe</td><td>2025-11-01</td><td>2025-11-05</td><td><span className="badge bg-warning">Reserved</span></td></tr>
//                 <tr><td>002</td><td>Jane Smith</td><td>Suite</td><td>2025-10-28</td><td>2025-11-03</td><td><span className="badge bg-success">Checked-In</span></td></tr>
//               </tbody>
//             </table>
//           </div>
//         </div>
//       </div>

//       <div className="row g-3">
//         <div className="col-md-6">
//           <div className="card shadow-sm">
//             <div className="card-body">
//               <h6>Quick Actions</h6>
//               <div className="d-flex gap-2 mt-3">
//                 <button className="btn btn-primary">New Booking</button>
//                 <button className="btn btn-outline-secondary">Add Room</button>
//               </div>
//             </div>
//           </div>
//         </div>
//         <div className="col-md-6">
//           <div className="card shadow-sm">
//             <div className="card-body">
//               <h6>Notifications</h6>
//               <ul className="list-unstyled mb-0">
//                 <li className="py-2 border-bottom">Room 102 requires cleaning.</li>
//                 <li className="py-2 border-bottom">Maintenance requested: Room 305.</li>
//               </ul>
//             </div>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// }

// export default Dashboard;
